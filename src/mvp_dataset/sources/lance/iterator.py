...
from .shuffle import (
    DEFAULT_CHUNK_AWARE_SHUFFLE_CHUNK_SIZE,
    DEFAULT_CHUNK_AWARE_SHUFFLE_K,
    ChunkAwareShuffleOrder,
    chunk_aware_index_order,
    fragment_aware_index_order,
    lance_index_at,
    lance_round_size,
)
...
class _LanceSourceIterator:
    ...
    _chunk_order: ChunkAwareShuffleOrder | None = None
    _chunk_order_round: int | None = None
    ...
    def state_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "chunk_aware_shuffle_chunk_size": self.chunk_aware_shuffle_chunk_size,
            "chunk_aware_shuffle_k": self.chunk_aware_shuffle_k,
    ...
    def load_state_dict(self, state: dict[str, object]) -> None:
        ...
        if state.get("shuffle_mode") != self.shuffle_mode:
            ...
            raise ResumeStateError(msg)
        if state.get("chunk_aware_shuffle_chunk_size") != self.chunk_aware_shuffle_chunk_size:
            msg = "[InvalidResumeState] chunk_aware_shuffle_chunk_size does not match"
            raise ResumeStateError(msg)
        if state.get("chunk_aware_shuffle_k") != self.chunk_aware_shuffle_k:
            msg = "[InvalidResumeState] chunk_aware_shuffle_k does not match"
            raise ResumeStateError(msg)
        ...
        self._index_order = []
        self._index_order_round = None
        self._chunk_order = None
        self._chunk_order_round = None
    ...
    def _round_size(self, round_index: int) -> int:
        ...
        chunk_order = self._chunk_order_for_round(round_index) if self.shuffle_mode == "chunk_aware" else None
        return lance_round_size(self.source, self.context, self.shuffle_mode, fragment_order, chunk_order)
    ...
    def _chunk_order_for_round(self, round_index: int) -> ChunkAwareShuffleOrder:
        """Return compact chunk-aware order metadata for one round."""
        if self._chunk_order_round == round_index and self._chunk_order is not None:
            return self._chunk_order

        if self.shuffle_mode == "chunk_aware":
            chunk_order = chunk_aware_index_order(
                self.source,
                self.context,
                round_index,
                chunk_size=self.chunk_aware_shuffle_chunk_size,
                k=self.chunk_aware_shuffle_k,
            )
        else:
            msg = (
                "[InvalidLanceShuffleMode] chunk order is only materialized for "
                f"chunk_aware, got {self.shuffle_mode!r}"
            )
            raise ValueError(msg)

        self._chunk_order = chunk_order
        self._chunk_order_round = round_index
        return self._chunk_order
